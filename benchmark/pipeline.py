...
class Method_MBRS(MethodBase):
    ...
    def _string_to_bits(self, msg: str) -> torch.Tensor:
        ...
        if msg_len > 255:
            raise ValueError("Message too long: max 255 bytes supported with 1-byte length prefix.")
    ...
    def encode(self, img_pil: Image.Image, msg: str) -> Image.Image:
        ...
        msg_bits = self._string_to_bits(msg).unsqueeze(0).to(self.device)  # [1, message_length]
