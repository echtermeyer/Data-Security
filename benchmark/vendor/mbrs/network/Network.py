...
class Network:
    ...
    def __init__(
        ...
        if not with_diffusion:
            self.encoder_decoder = EncoderDecoder(
                H, W, message_length, noise_layers
            ).to(device)
        else:
        ...
        self.label_cover = torch.full(
            (batch_size, 1), 1, dtype=torch.float, device=device
        )
        self.label_encoded = torch.full(
    ...
    def train(self, images: torch.Tensor, messages: torch.Tensor):
        ...
        with torch.enable_grad():
            ...
            d_cover_loss = self.criterion_BCE(
                d_label_cover, self.label_cover[: d_label_cover.shape[0]]
            )
            ...
            d_encoded_loss = self.criterion_BCE(
                d_label_encoded, self.label_encoded[: d_label_encoded.shape[0]]
            )
            ...
            g_loss_on_discriminator = self.criterion_BCE(
                g_label_decoded, self.label_cover[: g_label_decoded.shape[0]]
            )
            ...
            # ssim
            ssim = 1 - 2 * kornia.losses.ssim(
                encoded_images.detach(), images, window_size=5, reduction="mean"
            )
    ...
    def decoded_message_error_rate_batch(self, messages, decoded_messages):
        ...
        for i in range(batch_size):
            error_rate += self.decoded_message_error_rate(
                messages[i], decoded_messages[i]
            )
    ...
    def load_model_ed(self, path_encoder_decoder: str, device):
        self.encoder_decoder.module.load_state_dict(
            torch.load(path_encoder_decoder, map_location=device)
        )
