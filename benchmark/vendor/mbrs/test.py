...
test_result = {"error_rate": 0.0, "psnr": 0.0, "ssim": 0.0}

start_time = time.time()

saved_iterations = np.random.choice(
    np.arange(len(test_dataset)), size=save_images_number, replace=False
)
saved_all = None
...
for i, images in enumerate(test_dataloader):
    ...
    message = torch.Tensor(
        np.random.choice([0, 1], (image.shape[0], message_length))
    ).to(device)
    ...
    if i in saved_iterations:
        if saved_all is None:
            ...
            saved_all = concatenate_images(
                saved_all, image, encoded_images, noised_images
            )
