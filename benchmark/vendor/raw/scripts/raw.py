...
from vendor.raw.scripts.tools import wm_method
from vendor.raw.scripts.model_tools import get_model
...
class RAWatermark(object):
    ...
    def __init__(
        ...
        device="cpu",
        model_type="resnet18",
        save_dir="vendor/raw/assets/pre_trained",
        wm_index=0,
    ):

        ## setup watermark
        self.spa_watermark = torch.load(
            os.path.join(save_dir, "wm{}/wm_{}.pt".format(wm_index, wm_index)),
            map_location=torch.device(device),
        ).to(device)
        self.freq_watermark = torch.load(
            os.path.join(save_dir, "wm{}/spa_wm_{}.pt".format(wm_index, wm_index)),
            map_location=torch.device(device),
        ).to(device)
        self.wm_mask = torch.load(
            os.path.join(save_dir, "wm{}/wm_mask_{}.pt".format(wm_index, wm_index)),
            map_location=torch.device(device),
        ).to(device)
        ...
        self.classifier.load_state_dict(
            torch.load(
                os.path.join(save_dir, "wm{}/model_{}.pth".format(wm_index, wm_index)),
                map_location=torch.device(device),
            )
        )
    ...
    def encode(self, video, injection_every_k_frames=1):
        ...
        if len(shape) == 4 and shape[2] == shape[3] == 512 and shape[1] == 3:
            ...
            raise ValueError(
                "Input shape is not in the correct form. Currently, we only support videos in (number of frames, C = 3, H = 512, W = 512)"
            )
        ...
        unselected_frames = [
            ...
            if index % injection_every_k_frames != 0
        ]

        # Embed watermark and clamp values to a specified range
        wm_video_selected = wm_method(
            selected_frames,
            self.freq_watermark,
            self.spa_watermark,
            self.wm_mask,
        ...
        combined_frames = [
            (
                wm_video_selected[index // injection_every_k_frames]
                if index % injection_every_k_frames == 0
                else unselected_frames[index // injection_every_k_frames]
            )
            for index, _ in enumerate(video)
        ]
    ...
    def detect(self, video, decision_thres=0.5):
        ...
        print(
            "Watermark detected!"
            if torch.mean(torch.softmax(pred, dim=1)[:, 1]).item() > decision_thres
    ...
    def detect_img(self, img, decision_thres=0.5, prob=False):
        ...
        if prob:
            ...
            print(
                "Watermark detected!"
                if (torch.softmax(pred, dim=1)[:, 1]).item() > decision_thres
                else "Watermark not detected!"
            )
