...
parser.add_argument(
    "--visualize_detector", action="store_true", help="Visualize detector mask output"
)
...
def main():
    ...
    with detector_graph.as_default():
        ...
        detector_model = tf.saved_model.loader.load(
            detector_sess, [tag_constants.SERVING], args.detector_model
        )

        detector_input_name = (
            detector_model.signature_def[
    ...
    with decoder_graph.as_default():
        ...
        decoder_model = tf.saved_model.loader.load(
            decoder_sess, [tag_constants.SERVING], args.decoder_model
        )

        decoder_input_name = (
            decoder_model.signature_def[
                signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY
            ]
            .inputs["image"]
            .name
    ...
    while True:
        ...
        detector_image_input = (
            np.expand_dims(np.float32(detector_image_input), axis=0) / 255.0
        )
        ...
        for cnt in contours:
            ...
            for im_rotation in range(4):
                ...
                recovered_secret = decoder_sess.run(
                    [decoder_output], feed_dict={decoder_input: [w_rotated]}
                )[0][0]
                ...
                if bitflips != -1:
                    ...
                    im = cv2.putText(
                        frame,
                        code,
                        tuple((corners[0, :] + np.array([0, -15])).astype(np.int)),
                        font,
                        1,
                        (0, 0, 0),
                        2,
