...
def main(args, device):
    ...
    if args.load_text:  # text to bits
        if len(args.groundtruth_message) > 12:
            ...
            raise SystemExit
        data = bytearray(
            args.groundtruth_message + " " * (12 - len(args.groundtruth_message)),
            "utf-8",
        )
    ...
    groundtruth_watermark_list = (
        watermark[0].cpu().detach().numpy().astype(int).tolist()
    )

    same_elements_count = sum(
        x == y for x, y in zip(groundtruth_watermark_list, pred_watermark_list)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path",
        type=str,
        default="./example/edited_watermarked_img/2_wm_edit.png",
        help="path to the (edited) watermarked image",
    )
    parser.add_argument(
        "--pretrained_model_name",
        type=str,
        default="Shilin-LU/VINE-R-Dec",
    ...
    parser.add_argument(
        ...
        type=str,
        default="Hello World!",
        help="the secret message to be encoded",
    )
    parser.add_argument(
        "--load_text",
        type=bool,
        default=True,
        help="the flag to decide to use inputed text or random bits",
    )
